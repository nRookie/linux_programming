...
int del_cdt_entry(const char *cd_catalog_ptr, const int track_no)
{
	...
	if(send_mess_to_server(mess_send)){
		if(read_one_response == r_success){
			if(mess_ret.response == r_success){
...
cdc_entry search_cdc_entry(const char *cd_catalog_ptr, int *first_call_tr)
{
	message_db_t mess_send;
	message_db_t mess_ret;
	...
	if(*first_call_ptr){
		*first_call_ptr = 0;
		...
		fprintf(stderr,"Server not accpeting requests\n");
	}
