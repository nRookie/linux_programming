...
int main(int argc, char *argv[]){
	...
	if((sigaction(SIGINT,&new_action,&old_action)!=0) ||
			(sigaction(SIGHUP,&new_action,&old_action)!=0)||(sigaction(SIGTERM,&new_action,&old_action)!=0)){
...
static void process_command(const message_db_t comm)
{
	...
	switch(resp.request){
		...
		case s_get_cdt_entry:
			resp.cdt_entry_data = 
				get_cdt_entry(comm.cdt_entry_data.catalog,comm.cdt_entry_data.track_no);
		...
		case s_del_cdc_entry:
			if(!del_cdc_entry(comm.cdt_entry_data.catalog)) resp.response = r_failure;
		case s_del_cdt_entry:
			if(!del_cdt_entry(comm.cdt_entry_data.catalog,comm.cdt_entry_data.track_no)) resp.response = r_failure;
			break;
